...
- **Double-bezel mandatory:** every important card is a wrapper (`p-1.5`, hairline border, soft outer shadow) containing an inner core with its own background and a _concentric smaller radius_ (e.g., outer `2rem`, inner `calc(2rem - 0.375rem)`).
