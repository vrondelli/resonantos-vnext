...
Inspired by the _huashu-design_ tweak pattern.
...
Three settings: _Compact_ (0.85), _Normal_ (1.0), _Generous_ (1.15).
...
Three settings that swap the spacing scale: _Tight_ (0.75) /
_Normal_ (1.0) / _Roomy_ (1.4). All `padding` / `gap` / `margin`
...
If the artifact already has a media-query-based dark mode, _replace_
...
- _Off_ — `0s` (also disables WebGL canvases / decorative animation).
- _Subtle_ — `1.0` (the artifact's authored timing).
- _Lively_ — `1.6` (slower transitions, more visible motion).

Respect `prefers-reduced-motion`: default to _Off_ if the user has
...
4. Edit `assets/wrap.html`'s `KNOBS` array to keep only the knobs
   you decided are relevant to _this_ artifact. Don't ship 5 if 2
   matter.
...
Read the artifact's CSS first. For each knob, decide _yes / no_:
...
_outer_ value by the custom property — don't tear apart `clamp(...)`.
...
- **Respect `prefers-reduced-motion`** — default to _Off_ for motion
