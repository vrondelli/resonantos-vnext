...
the _huashu-design_ expert-critique flow.
...
3. **Five dimension cards**, each with:
   - Score 0–10 (with band: 0–4 _Broken_ · 5–6 _Functional_ · 7–8 _Strong_
     · 9–10 _Exceptional_)
...
> Does the artifact pick a clear _direction_ and stick to it through
...
- Do mono / serif / sans roles match the information's _role_ (meta /
  body / display)?
...
> Does the artifact _work_ for its intended use? Click targets, nav,
...
- One _unexpected_ layout / motion / typographic move that wasn't
  required?
- Or 100% safe — could be any deck/landing from any agency?
- Is the innovation _earned_ (matches direction) or grafted on
...
- **Don't average up** — if Hierarchy is 5 because page 3 is broken,
  ...
  _worst sustained band_.
- **Don't grade-inflate** — a 7 means _strong_, not _acceptable_. If
  every score is 7+, you're not reviewing critically.
- **Innovation is allowed to be low** — 5/10 is fine for production
  deliverables. Don't punish _appropriate_ conservatism.
...
_executed_ design, not declared intent.
...
- **Fix** (3–6 bullets) — must-do, ordered by _visual cost saved per
  minute spent_. Each bullet ≤ 1 sentence.
